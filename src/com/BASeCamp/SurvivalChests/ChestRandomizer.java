...
public class ChestRandomizer {
	...
	public int Shuffle() {
		...
		if(gotinventory instanceof PlayerInventory)
		{
			PlayerInventory pi = (PlayerInventory)gotinventory;
			ItemStack[] ArmorItems = pi.getArmorContents();
			for(int i=0;i<ArmorItems.length;i++)
			{
				if(ArmorItems[i]!=null)
				    pi.addItem(ArmorItems[i]);
			}
			...
			pi.setItemInOffHand(RandomData.GenerateShield());
		}
